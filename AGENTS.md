...
3. **5 шагов** (steps-grid):
   ...
   - Шаг 4: Выбор ключевой пары (select + кнопки «Обновить»/«Удалить»)
   - Шаг 5: Подпись данных (textarea + кнопка «Подписать» + результат)
4. **Журнал операций** — обычные и подробные логи, кнопка копирования
...
| **5 шагов** | Выбор устройства → PIN → создание → выбор ключа → подпись |
| **Журнал** | Копируемая трассировка API; PIN и содержимое данных не записываются |
...
2. **Нужен повторный тест с реальным токеном** — аппаратно-зависимые ветви биометрии после изменений должны быть подтверждены
3. **Несколько наборов отпечатков** — выбор `convolutionsId` пока не реализован
